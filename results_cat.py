...
#             variable_name = f"{line}_{data_type}"
...
#         if not os.path.exists(em_lines_path):
#             print(f"File not found: {em_lines_path}. Galaxy ID: {ID} will not be included in the results table.")
#             continue

#         em_lines = Table.read(em_lines_path, format='fits')

#         # for i in range(len(lines)):
#         #     row = em_lines[i]
#         #     if row[0] != lines[i]:
#         #         print(f"Line mismatch: {row[0]} != {lines[i]}. Galaxy ID: {ID}.")
#         #     if row[0] == lines[i]:
#         #         name = row[0]
#         #         wave = row[1]
#         #         flux = row[2]
...
#             #find the row of the catalogue where row[0] == target_line
#             for i in range(len(em_lines)):
#                 row = em_lines[i]
#                 if row[0] == target_line:

#                     print(f"Line match: {row[0]} == {target_line}. Galaxy ID: {ID}.")
...
#                     wave_list_name = f"{lines[i]}_wave_list"
#                     flux_list_name = f"{lines[i]}_flux_list"
#                     flux_err_list_name = f"{lines[i]}_flux_err_list"
#                     flux_mc_err_list_name = f"{lines[i]}_flux_mc_err_list"
#                     flux_upper_list_name = f"{lines[i]}_flux_upper_list"
#                     ew_rest_list_name = f"{lines[i]}_ew_rest_list"
#                     ew_rest_err_list_name = f"{lines[i]}_ew_rest_err_list"
#                     flux_flag_list_name = f"{lines[i]}_flux_flag_list"
#                     vel_list_name = f"{lines[i]}_vel_list"
#                     vel_err_list_name = f"{lines[i]}_vel_err_list"
#                     sig_list_name = f"{lines[i]}_sig_list"
#                     sig_err_list_name = f"{lines[i]}_sig_err_list"
#                     comp_list_name = f"{lines[i]}_comp_list"

#                     locals()[wave_list_name].append(wave)
#                     locals()[flux_list_name].append(flux)
#                     locals()[flux_err_list_name].append(flux_err)
#                     locals()[flux_mc_err_list_name].append(flux_mc_err)
#                     locals()[flux_upper_list_name].append(flux_upper)
#                     locals()[ew_rest_list_name].append(ew_rest)
#                     locals()[ew_rest_err_list_name].append(ew_rest_err)
#                     locals()[flux_flag_list_name].append(flux_flag)
#                     locals()[vel_list_name].append(vel)
#                     locals()[vel_err_list_name].append(vel_err)
#                     locals()[sig_list_name].append(sig)
#                     locals()[sig_err_list_name].append(sig_err)
#                     locals()[comp_list_name].append(comp)
...
#             for data_type in data_types:
#                 variable_name = f"{line}_{data_type}"
#                 col_name = variable_name.replace("_list", "")
#                 process_column(hdul, col_name, locals()[variable_name], output_path, update_existing=False)

#     print(f"Results table successfully written to {output_path}")
